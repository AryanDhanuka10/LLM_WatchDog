...
def make_readme(root: Path) -> None:
    write(root / "README.md", """
        ...
        from llm_meter import watchdog
...
def make_prices_json(root: Path) -> None:
    ...
    path = root / "src" / "llm_meter" / "pricing" / "prices.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def make_config(root: Path) -> None:
    write(root / "src" / "llm_meter" / "config.py", """
        ...
        Usage:
            ...
            from llm_meter.config import configure
...
def make_db(root: Path) -> None:
    write(root / "src" / "llm_meter" / "storage" / "db.py", """
        \"\"\"
...
def make_providers(root: Path) -> None:
    write(root / "src" / "llm_meter" / "providers" / "base.py", """
        \"\"\"
    ...
    write(root / "src" / "llm_meter" / "providers" / "openai.py", """
        \"\"\"
    ...
    write(root / "src" / "llm_meter" / "providers" / "anthropic.py", """
        \"\"\"
...
def make_pricing(root: Path) -> None:
    write(root / "src" / "llm_meter" / "pricing" / "table.py", """
        \"\"\"
...
def make_core(root: Path) -> None:
    for name, day in [
        ...
        write(root / "src" / "llm_meter" / "core" / name, f"""
            \"\"\"
            {name} — PLACEHOLDER
...
def make_cli(root: Path) -> None:
    write(root / "src" / "llm_meter" / "cli" / "__main__.py", """
        \"\"\"
    ...
    write(root / "src" / "llm_meter" / "cli" / "commands.py", """
        \"\"\"
...
def make_init(root: Path) -> None:
    write(root / "src" / "llm_meter" / "__init__.py", """
        ...
        # Day 3: from llm_meter.core.decorator import watchdog
        # Day 3: from llm_meter.core.context import watch
        # Day 4: from llm_meter.core.budget import Budget
        # Day 4: from llm_meter.exceptions import BudgetExceeded
        # Day 7: from llm_meter.core.interceptor import intercept
    """)
...
def make_tests(root: Path) -> None:
    ...
    write(root / "tests" / "conftest.py", """
        ...
        from llm_meter.config import configure, reset_config
    ...
    write(root / "tests" / "unit" / "test_placeholder.py", """
        ...
        def test_package_importable():
            ...
            import llm_meter
            assert llm_meter.__version__ == "0.1.0"
        ...
        def test_exceptions_importable():
            ...
            from llm_meter.exceptions import (
                WatchdogError,
                BudgetExceeded,
                ProviderNotDetected,
...
def scaffold(base_path: str = ".") -> None:
    ...
    for p in ["__init__.py"]:
        touch(root / "src" / "llm_meter" / "storage" / p)

    print("\n── Providers ────────────────────────────────────")
    make_providers(root)
    for p in ["__init__.py"]:
        touch(root / "src" / "llm_meter" / "providers" / p)

    print("\n── Pricing ──────────────────────────────────────")
    make_pricing(root)
    make_prices_json(root)
    touch(root / "src" / "llm_meter" / "pricing" / "__init__.py")

    print("\n── Core (decorators, budget, interceptor) ───────")
    make_core(root)
    touch(root / "src" / "llm_meter" / "core" / "__init__.py")
